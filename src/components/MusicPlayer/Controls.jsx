...
const Controls = ({ isPlaying, repeat, setRepeat, shuffle, setShuffle, currentSongs, handlePlayPause, handlePrevSong, handleNextSong }) => (
  <div className="flex items-center justify-around md:w-36 lg:w-52 2xl:w-80">
    <BsArrowRepeat size={20} color={repeat ? 'white' : 'gray'} onClick={() => setRepeat((prev) => !prev)} className="hidden sm:block cursor-pointer" />
    ...
    <BsShuffle size={20} color={shuffle ? 'white' : 'gray'} onClick={() => setShuffle((prev) => !prev)} className="hidden sm:block cursor-pointer" />
  </div>
);
